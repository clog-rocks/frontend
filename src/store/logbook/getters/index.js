...
export default {
  ...
  CRAG_VISIT_COUNT: (state) =>
    [...new Set(Object.values(state.data).map((x) => x.date))].length,

  CRAG_VISIT_LAST_YEAR_COUNT: (_state, getters) =>
    [...new Set(Object.values(getters.ASCENT_LAST_YEAR).map((x) => x.date))]
      .length,

  TOP_GRADES: (state) => {
    const get_max_grade_for_style = (style) =>
      state.grades[
        Math.max(
